  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (result.text && !result.text.startsWith('Something went wrong')) {
        addEntry({ brand: brand.short, toolName: selectedTool.name, type: 'intel', fullOutput: result.text });
      }
    } catch (e) {
    ...
    return (
      <div className="p-5 md:p-8 max-w-5xl mx-auto space-y-6">
        <button onClick={handleBack} className="flex items-center gap-2 text-[13px] text-muted-foreground hover:text-foreground transition-premium bg-transparent border-none cursor-pointer">
          <ArrowLeft className="h-4 w-4" /> Back to SEO tools
        </button>
        <div className="flex items-center gap-3">
          ...
          {selectedTool.badge && (
            <Badge variant="outline" className="text-[10px] ml-auto">{selectedTool.badge}</Badge>
          )}
        </div>
                ...
                <div key={field.id} className="space-y-1.5">
                  ...
                  {field.type === 'select' ? (
                    <Select value={inputs[field.id] || ''} onValueChange={(v) => setInputs((p) => ({ ...p, [field.id]: v }))}>
                      ...
                      <SelectContent>
                        {field.options?.map((opt, i) => <SelectItem key={opt} value={opt}>{field.labels?.[i] || opt}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  ) : (
          ...
          <Card>
            <CardHeader className="pb-4">
              <div className="flex items-center justify-between">
                <CardTitle className="text-metadata text-muted-foreground">Results</CardTitle>
            ...
            <CardContent>
              {loading ? (
                <div className="flex items-center justify-center h-48 text-[13px] text-muted-foreground"><Loader2 className="h-5 w-5 mr-2 animate-spin" /> Analyzing…</div>
              ) : output ? (
                <div className="prose prose-sm max-w-none whitespace-pre-wrap text-[13px] text-foreground/90 leading-relaxed">{output}</div>
              ) : (
                <div className="flex items-center justify-center h-48 text-[13px] text-muted-foreground">Results will appear here</div>
              )}
            </CardContent>
          </Card>
        </div>
  ...
  return (
    <div>
      ...
      <div className="p-5 md:p-8 max-w-6xl mx-auto space-y-6">
        <div>
          <h2 className="text-metadata text-muted-foreground mb-3">Intelligence Tools</h2>
        ...
        <div>
          <h2 className="text-metadata text-muted-foreground mb-3">Coming Soon</h2>
